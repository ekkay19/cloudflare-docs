...
Wait for a process to exit and retrieve the exit code.
...
Use `getLogs()` or `streamProcessLogs()` to retrieve output after the process exits.
...
// Retrieve logs after exit
const logs = await build.getLogs();
console.log('Build output:', logs.stdout);
...
if (testResult.exitCode !== 0) {
  const testLogs = await tests.getLogs();
  throw new Error(`Tests failed: ${testLogs.stderr}`);
}
```
</TypeScriptExample>
