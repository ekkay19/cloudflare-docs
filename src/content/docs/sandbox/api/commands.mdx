...
- `waitForExit()` - Wait for process to exit and get exit code with output
...
Wait for a process to complete and return its exit code along with output.
...
Useful for processes that complete on their own (builds, scripts, tests) rather than long-running services. Unlike `exec()`, this method is designed for background processes started with `startProcess()`.
...
const result = await build.waitForExit(60000);
...
// Run tests and validate exit code
const tests = await sandbox.startProcess('npm test');
const testResult = await tests.waitForExit();
