...
If you need to start a process in the background and wait for it to complete (for example, to run parallel builds or tests), use `startProcess()` with `waitForExit()`.
:::
...
For processes that complete on their own (builds, tests, scripts), use `waitForExit()` to wait for them to finish and get their exit code:
...
const result = await build.waitForExit(300000); // 5 minute timeout
...
This is useful for running multiple builds or tests in parallel:

<TypeScriptExample>
```
// Start multiple test suites in parallel
const unit = await sandbox.startProcess('npm run test:unit');
const integration = await sandbox.startProcess('npm run test:integration');
const e2e = await sandbox.startProcess('npm run test:e2e');
...
// Check results
const allPassed = [unitResult, integrationResult, e2eResult].every(r => r.exitCode === 0);
...
- **Wait for completion** - Use `waitForExit()` for processes that complete on their own (builds, tests, scripts)
