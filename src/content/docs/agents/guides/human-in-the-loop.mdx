...
:::note
**Client-side tools** (with an `execute` function) run in the browser and can either execute automatically or require human confirmation based on your `toolsRequiringConfirmation` configuration. **Server-side tools** (without an `execute` function) run on the server after receiving approval.

When defining client-side tools, use JSON Schema for the `parameters` field instead of Zod, as the tool definitions need to be serialized and sent over the network.
:::
...
type InferArgs<T extends { inputSchema?: { parse: (args: unknown) => unknown } }> =
  T extends { inputSchema: { parse: (args: unknown) => infer R } } ? R : never;

// Process tool calls after user confirmation
export async function processToolCalls<
  Tools extends ToolSet,
  K extends keyof Tools & string = keyof Tools & string
>(
  {
    messages,
    tools: _tools
  ...
  executeFunctions: {
    [key in K]?: (args: InferArgs<Tools[key]>) => Promise<string>;
  }
): Promise<UIMessage[]> {
  ...
  const updatedParts = await Promise.all(
    lastMessage.parts.map(async (part) => {
      if (isToolConfirmationPart(part) && part.type.startsWith("tool-")) {
        const toolName = part.type.slice("tool-".length) as K;
...
The agent uses the Vercel AI SDK streaming capabilities:

- `streamText` handles AI responses with tool calls.
- The `hasToolConfirmation` function detects when a message contains a tool confirmation response.
- After processing confirmations, the agent continues with `streamText` to let the LLM respond to the tool result.

### Auto-continuation after tool results

The `autoContinueAfterToolResult` option enables seamless tool execution flow:

1. **Client executes the tool** and sends the result to the server (via `CF_AGENT_TOOL_RESULT` message).
2. **Server automatically calls `onChatMessage()`** to continue the conversation.
3. **The LLM continuation is merged** into the same assistant message.
4. **User sees a single, seamless response** instead of a fragmented conversation.

Without this option, client-side tool execution would require the user to send another message for the assistant to continue. With this option enabled, the flow is automatic and mimics server-side tool behavior.
...
For server-side tools requiring confirmation, add execute functions to the `processToolCalls` call in your agent:
...
if (hasToolConfirmation(lastMessage)) {
  ...
  // Continue with streamText so the LLM can respond to the tool result
  const result = streamText({
    messages: convertToModelMessages(this.messages),
    model: openai("gpt-4o"),
    onFinish,
