...
type UseAgentChatOptions = Omit<
	Parameters<typeof useChat>[0] & {
		...
		// Client-side tools with optional execute functions
		tools?: Record<string, AITool>;
		// List of tool names that require human confirmation before execution
		toolsRequiringConfirmation?: string[];
		// When true, automatically continues the conversation after client-side tool execution
		// This mimics server-side tool behavior for a seamless user experience
...
#### Client-side tools with human-in-the-loop

`useAgentChat` supports client-side tools with optional human confirmation workflows. This is useful when you want tools to execute in the browser (for example, accessing local storage, camera, or geolocation) while maintaining control over which tools require user approval.
...
// Define client-side tools using JSON Schema
const clientTools: Record<string, AITool> = {
  ...
  sendEmail: {
    description: "Send an email (server-side only, requires approval)"
    // No execute function - handled by server after approval
  }
};
...
function ChatWithTools() {
  ...
  const { messages, addToolResult } = useAgentChat({
    ...
    toolsRequiringConfirmation: ["getLocation", "sendEmail"],
    // Automatically continue after tool execution for seamless UX
      ...
      {messages.map((message) => (
        <div key={message.id}>
          {message.parts?.map((part) => {
            // Show approval UI for tools requiring confirmation
...
**Client-side tools** (with `execute` function) run in the browser. **Server-side tools** (without `execute`) run on the server after approval. Use JSON Schema for the `parameters` field, as tool definitions are serialized over the network.
...
- [Build a Human-in-the-Loop Agent](/agents/guides/human-in-the-loop/) with client-side tools and approval workflows.
